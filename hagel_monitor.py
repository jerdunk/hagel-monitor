...
def _find_2d(node):
    """Zoek recursief naar de eerste 2D dataset in een HDF5 groep."""
    for k in node:
        it = node[k]
...
#!/usr/bin/env python3
"""Hagelmonitor voor jeroendunk.nl - draait via GitHub Actions (elke 5 min).
Bronnen (KNMI Data Platform, CC-BY-4.0):
  cell-tracking 2.0 (CellWarn), radar_hail_warning_5min 1.0, waarschuwingen_nederland_48h 1.0
...
def read_hail_hdf5(content):
    ...
    try:
        with h5py.File(path, "r") as h5:
            arr = None
            for k in ("image1", "dataset1"):
                if k in h5:
                    g = h5[k]
                    for s in ("image_data", "data1"):
                        if s in g:
                            n = g[s]; arr = n["data"][:] if "data" in n else n[:]; break
            if arr is None:
                def f2d(g):
                    for k in g:
                        it = g[k]
                        if isinstance(it, h5py.Dataset) and it.ndim == 2: return it[:]
...
def to_grid(values):
    ...
    ys = np.linspace(0, win.shape[0], GRID_ROWS+1).astype(int)
    xs = np.linspace(0, win.shape[1], GRID_COLS+1).astype(int)
    return [[float(np.nanmax(win[ys[i]:ys[i+1], xs[j]:xs[j+1]]) or 0)
